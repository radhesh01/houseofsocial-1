...
<style>
    /* ================================================================
   BLOG DETAIL PAGE — HouseOfSocial
================================================================ */
    .bd-hero {
        background: var(--s0);
        min-height: 60svh;
    ...
    .bd-hero-glow {
        ...
        width: 500px;
        height: 500px;
        background: radial-gradient(circle, rgba(255, 60, 0, .07) 0%, transparent 65%);
        filter: blur(80px);
        pointer-events: none;
        z-index: 0;
    }
    ...
    .bd-hero-content {
        ...
        max-width: var(--maxW);
        margin: 0 auto;
        padding: calc(var(--navH) + 56px) var(--px) clamp(40px, 7vw, 80px);
        width: 100%;
        margin-top: auto;
    }
    ...
    .bd-rel-card:hover .bd-rel-title {
        color: var(--flame);
    }
    ...
    @media (max-width:640px) {
        ...
        .bd-action-area .btn-primary,
        .bd-action-area .btn-outline {
            width: 100%;
...
<article>
    <header class="bd-hero">
        <div class="bd-hero-grid" aria-hidden="true"></div>
    ...
    <div class="bd-layout">
        ...
        <aside class="bd-sidebar" aria-label="Article metadata">
            ...
            <div class="bd-sidebar-card rv sl d2">
                ...
                <p style="font-size:13px;color:var(--ghost4);line-height:1.75;margin-bottom:18px">
                    Talk to us about building a campaign that actually moves culture.
                </p>
                <a href="<?= base_url('contact') ?>" class="btn-primary" style="width:100%;justify-content:center">Let's
                    Talk</a>
            </div>
            ...
            <div class="bd-sidebar-card rv sl d3">
                ...
                <a href="<?= base_url('blog') ?>"
                    style="display:block;font-size:13px;color:var(--ghost4);margin-bottom:8px;transition:color .2s"
                    onmouseover="this.style.color='var(--flame)'" onmouseout="this.style.color='var(--ghost4)'">← All
                    Articles</a>
                <a href="<?= base_url('work') ?>"
                    style="display:block;font-size:13px;color:var(--ghost4);margin-bottom:8px;transition:color .2s"
                    onmouseover="this.style.color='var(--flame)'" onmouseout="this.style.color='var(--ghost4)'">Our
                    Work</a>
                <a href="<?= base_url('services') ?>"
                    style="display:block;font-size:13px;color:var(--ghost4);transition:color .2s"
                    onmouseover="this.style.color='var(--flame)'" onmouseout="this.style.color='var(--ghost4)'">Our
                    Services</a>
            </div>
        </aside>
...
<?php
// Load recent blog posts for related section (exclude current)
$CI = &get_instance();
            ...
            <div class="bd-related-grid rv sc">
                <?php foreach ($related as $r): ?>
                    <a href="<?= base_url('blog/' . $r['slug']) ?>" class="bd-rel-card">
                        <div class="bd-rel-meta"><?= htmlspecialchars($r['author']) ?> ·
                            <?= date('d M Y', strtotime($r['created_at'])) ?></div>
                        <h3 class="bd-rel-title"><?= htmlspecialchars($r['title']) ?></h3>
                        <?php if (!empty($r['subtitle'])): ?>
                            <p
                                style="font-size:12px;color:var(--ghost3);line-height:1.65;margin-top:8px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">
